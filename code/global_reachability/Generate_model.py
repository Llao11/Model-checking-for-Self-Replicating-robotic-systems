...
class Generate_model:
    def __init__(self):
        ...
        self.template_file = "Assemble_initial_template.smv"
        self.model_file = "Assemble_temporary.smv"
        with open(self.template_file, "r") as file:
            self.nusmv_model_template = file.read()

    def setup(self, field_size):
        # INITIAL PARAMETERS
        self.field_size = field_size
        self.field_max_index = field_size - 1
    ...
    def generate(self, field_values, robot_sequence, object_types):
        ...
        self.robot_sequence = robot_sequence

        # Types of objects on the field {NONE, TYPE1, TYPE2, OBSTACLE1, TYPE3}
        self.object_types = str(object_types).replace(
            "'", ""
        )  # f" {set(field_values)}".replace("'","")
    ...
    def verify(self):
        ...
        result = subprocess.run(
            ["../NuSMV-2.7.0-linux64/bin/NuSMV", "-dynamic", self.model_file],
            capture_output=True,
            text=True,
        )
        ...
        matching_lines = [
            line for line in result_analysis.splitlines() if pattern.search(line)
        ]
