...
class RobotController(Node):
    def __init__(self):
        ...
        [self.command_sequences_detachable, self.command_sequences_deg] = (
            self.read_commands()
        )
        ...
        self.command_sequences = [
            [element * math.pi / 180.0 for element in sublist]
            for sublist in self.command_sequences_deg
        ]
        self.step = 0
    ...
    def rotate_joins(self):
        for joint_index in range(len(self.command_sequences[0])):
            ...
            self.get_logger().info(
                f"Published command for joint {joint_index + 1}: {command.data}"
            )
