...
class SRRSController(Node):
    ...
    def rotate_joints(self, command_sequences):
        for joint_index in range(len(command_sequences)):
            self.rotate_joint(joint_index, float(
                command_sequences[joint_index]))
    ...
    def waiting_step(self):
        ...
        joint_angles_current_deg = [float(
            angle)*180.0/math.pi for angle in self.joint_angles_current]          # rad to deg
        # calculate difference in deg
        diff = [abs(a-b)
                for a, b in zip(joints_angles_target, joint_angles_current_deg)]
    ...
    def joint_state_changed(self, msg):
        ...
        self.joint_angles_current = [
            joint_angles_current_dict[i] for i in sorted_names]
