...
def on_set():
    ...
    part_list = [p.strip() for p in part_types_str.replace(',', ' ').split() if p.strip()]
    ...
    canvas.grid(row=0, column=0, columnspan=field_size_val, rowspan=field_size_val, sticky="nsew")
    ...
    for r in range(field_size_val):
        for c in range(field_size_val):
            ...
            # Create a button
            btn = tk.Menubutton(center_grid_frame, text=var.get(), relief=tk.RAISED, width=2, height=1)
            menu = tk.Menu(btn, tearoff=0)
            for index, part in enumerate(part_list):
                menu.add_command(label=part, command=lambda v=var, b=btn, p=part: update_cell(v, b, p))
...
def on_generate():
    set_output_text("Generate button pressed")

def on_verify():
    """ Draw predefined lines on the central grid """
    set_output_text("Verification successful! Drawing paths...")

    # Define line coordinates (grid positions)
    lines = [((0, 0), (0, 3)), ((0, 3), (2, 3))]

    # Draw the lines on the canvas
    for (x1, y1), (x2, y2) in lines:
        draw_line_between_cells(x1, y1, x2, y2)
...
def draw_line_between_cells(x1, y1, x2, y2):
    ...
    if (x1, y1) in cell_buttons and (x2, y2) in cell_buttons:
        ...
        canvas.create_line(x1_canvas, y1_canvas, x2_canvas, y2_canvas, fill="black", width=3)
...
root = tk.Tk()
root.title("Robot Part Selector")
...
# Output Field
output_text = tk.Text(left_frame, height=3, width=30, state=tk.DISABLED, wrap=tk.WORD)
